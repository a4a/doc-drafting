...
expression that describes the simplest type of population dynamics in the \a4a model
...
This equation, like (\ref{eq:survey_index}) gives predicted catches based on the population model
and provides a value which can be compared to the observed catches $C^\obs_{ay}$.  Taken together,
the comparison of the observations of catches and survey index to their predictions is the
underlying approach for estimating the numbers of fish in the population and the fishing mortality
rate.
